...
export function EventDetailView({
  ...
  // Gates the reveal of everything below the gallery on its lead photo's
  // actual load (see SpotDetailView/ArticleDetailView for the same pattern)
  // rather than firing on mount — with no photo to wait on, there's nothing
  // to gate, so start "ready".
  const [galleryReady, setGalleryReady] = useState(photos.length === 0);
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-6 pb-24 sm:px-6 sm:py-10 sm:pb-10">
      ...
      <div className="mt-6 grid gap-8 lg:mt-10 lg:grid-cols-[1fr_360px]">
        <Stagger className="space-y-8" show={galleryReady} delay={0.15}>
          <StaggerItem>
        ...
        <aside className="h-fit space-y-4 lg:sticky lg:top-24">
          <Stagger className="space-y-4" show={galleryReady} delay={0.15}>
            <StaggerItem>
